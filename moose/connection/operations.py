...
class BaseOperation(object):
    ...
    def __init__(self, handler, table_alias):
        if not isinstance(table_alias, dict):
            raise ImproperlyConfigured(\
                "Argument `table_alias` is the map of aliases to table names "
                "in database, which must be an instance of `dict`.")
    ...
    def create_from_context(cls, query_context):
        ...
        if handler_settings.get(cls.table_alias_name):
            table_alias = handler_settings[cls.table_alias_name]
        else:
            raise ImproperlyConfigured(\
                "Field `{}` is missing in sql context".format(cls.table_alias_name))
...
class BaseGuidQuery(BaseQuery):
    ...
    def __init__(self, handler, table_alias):
        ...
        self.operation_template = string.Template(self.operation_template).substitute(
            tables = self.tables,
